 ...
 * Purpose: Receiving / put-away draft workflow
 */
...
public partial class ReceivingPutAwayPage : ContentPage
{
    ...
    public ReceivingPutAwayPage(DatabaseService dbService, ScanService scanService, WorkflowExportService exportService, AcumaticaWorkflowService workflowService)
    {
        InitializeComponent();
        ...
        EndpointEntry.Text = "PurchaseReceipt";
    }
    ...
    {
        _drafts = await _dbService.GetDocumentDraftsAsync(DocumentDraftType.ReceivingPutAway);
    ...
    private async void OnCreateDraftClicked(object sender, EventArgs e)
    {
        ...
        {
            DraftType = DocumentDraftType.ReceivingPutAway,
            ReferenceNumber = PoNumberEntry.Text?.Trim() ?? string.Empty,
    ...
    private async void OnDraftSelected(object sender, EventArgs e)
    {
        ...
        _currentDraft = draft;
        PoNumberEntry.Text = draft.ReferenceNumber;
    ...
    {
        var qtyText = await DisplayPromptAsync("Quantity", "Enter received quantity:", keyboard: Keyboard.Numeric);
        ...
        var location = await DisplayPromptAsync("Location", "Suggested bin/location (optional):");
    ...
    private async void OnExportClicked(object sender, EventArgs e)
    {
        ...
        var csv = BuildCsv();
        var fileName = $"Receiving_{_currentDraft.ReferenceNumber}_{DateTime.UtcNow:yyyyMMdd_HHmmss}.csv";
        var path = await _exportService.SaveCsvAsync(fileName, csv);
        await _exportService.ShareFileAsync("Receiving Export", path);
    }
    ...
    private async void OnPushClicked(object sender, EventArgs e)
    {
        ...
        var endpoint = string.IsNullOrWhiteSpace(EndpointEntry.Text) ? "PurchaseReceipt" : EndpointEntry.Text.Trim();
