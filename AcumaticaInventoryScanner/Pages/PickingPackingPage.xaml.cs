 ...
 * Purpose: Picking / packing draft workflow
 */
...
public partial class PickingPackingPage : ContentPage
{
    ...
    public PickingPackingPage(DatabaseService dbService, ScanService scanService, WorkflowExportService exportService, AcumaticaWorkflowService workflowService)
    {
        InitializeComponent();
    ...
    {
        _drafts = await _dbService.GetDocumentDraftsAsync(DocumentDraftType.PickingPacking);
    ...
    private async void OnCreateDraftClicked(object sender, EventArgs e)
    {
        ...
        {
            DraftType = DocumentDraftType.PickingPacking,
            ReferenceNumber = OrderEntry.Text?.Trim() ?? string.Empty,
    ...
    private async void OnDraftSelected(object sender, EventArgs e)
    {
        ...
        _currentDraft = draft;
        OrderEntry.Text = draft.ReferenceNumber;
    ...
    {
        var qtyText = await DisplayPromptAsync("Quantity", "Enter picked quantity:", keyboard: Keyboard.Numeric);
        ...
        var location = await DisplayPromptAsync("Location", "Pick bin/location (optional):");
    ...
    private async void OnExportClicked(object sender, EventArgs e)
    {
        ...
        var csv = BuildCsv();
        var fileName = $"Picking_{_currentDraft.ReferenceNumber}_{DateTime.UtcNow:yyyyMMdd_HHmmss}.csv";
        var path = await _exportService.SaveCsvAsync(fileName, csv);
        await _exportService.ShareFileAsync("Picking Export", path);
    }
    ...
    private async void OnPushClicked(object sender, EventArgs e)
    {
        ...
        var endpoint = string.IsNullOrWhiteSpace(EndpointEntry.Text) ? "Shipment" : EndpointEntry.Text.Trim();
